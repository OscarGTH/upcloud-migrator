...
pub struct AwsResourceMapper;

impl ResourceMapper for AwsResourceMapper {
    fn map(&self, res: &TerraformResource) -> MigrationResult {
        match res.resource_type.as_str() {
            ...
            "aws_eip" => network::map_eip(res),
            "aws_eip_association" => network::map_eip_association(res),

            // Load Balancers
            "aws_lb" | "aws_alb" => loadbalancer::map_lb(res),
            "aws_lb_target_group" | "aws_alb_target_group" => {
                loadbalancer::map_lb_target_group(res)
            }
            "aws_lb_listener" | "aws_alb_listener" => loadbalancer::map_lb_listener(res),
            "aws_lb_target_group_attachment" | "aws_alb_target_group_attachment" => {
                loadbalancer::map_lb_target_group_attachment(res)
            }
            "aws_acm_certificate" => loadbalancer::map_acm_certificate(res),

            // Databases
            "aws_db_instance" | "aws_rds_instance" => database::map_rds_instance(res),
            "aws_rds_cluster" => database::map_rds_cluster(res),
            "aws_db_parameter_group" => database::map_db_parameter_group(res),
            "aws_db_subnet_group" => database::map_db_subnet_group(res),
            "aws_elasticache_cluster" | "aws_elasticache_replication_group" => {
                database::map_elasticache_cluster(res)
            }
